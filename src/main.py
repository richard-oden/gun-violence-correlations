...
from enums.ColumnNames import ColumnNames, regulation_column_names
...
gun_deaths_df.rename(columns={
    'Unnamed: 3': ColumnNames.COUNTRY.value,
    'Rate.3': ColumnNames.DEATH_RATE.value
}, inplace=True)

# Drop rows with no country.
gun_deaths_df.dropna(subset=[ColumnNames.COUNTRY.value], inplace=True)
...
gun_laws_df.rename(columns={
    'Region': ColumnNames.COUNTRY.value,
    'Good reason required?[3]': ColumnNames.GOOD_REASON.value,
    'Personal protection': ColumnNames.PERSONAL_PROTECTION.value,
    'Long guns (exc. semi- and full-auto)[4]': ColumnNames.LONG_GUNS.value,
    'Handguns[5]': ColumnNames.HANDGUNS.value,
    'Semi-automatic rifles': ColumnNames.SEMIAUTOMATIC.value,
    'Fully automatic firearms[6]': ColumnNames.FULLY_AUTOMATIC.value,
    'Open carry[7]': ColumnNames.OPEN_CARRY.value,
    'Concealed carry[8]': ColumnNames.CONCEALED_CARRY.value,
    'Free of registration[1]': ColumnNames.FREE_OF_REGISTRATION.value
}, inplace=True)

# Drop rows that represent subheadings.
gun_laws_df = gun_laws_df[gun_laws_df[ColumnNames.COUNTRY.value] != 'Region']
...
def get_country_name(gun_laws_row: pd.Series) -> str | None:
    ...
    '''
    return next((country_name for country_name in gun_deaths_df[ColumnNames.COUNTRY.value].tolist() 
        if country_name.lower().strip() in gun_laws_row[ColumnNames.COUNTRY.value].lower()), None)

gun_laws_df[ColumnNames.COUNTRY.value] = gun_laws_df.apply(get_country_name, axis=1)

merged_df = pd.merge(gun_deaths_df, gun_laws_df, how='inner', on=ColumnNames.COUNTRY.value)
...
def convert_to_regulations(df: pd.DataFrame) -> None:
    ...
    for column_name in regulation_column_names:
        is_restriction = column_name is ColumnNames.GOOD_REASON
...
merged_df[ColumnNames.OVERALL_REGULATION.value] = merged_df.apply(get_mean_regulation, axis=1)
